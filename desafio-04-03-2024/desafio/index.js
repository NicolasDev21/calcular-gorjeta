...
function payment() {
    ...
    fileSystem.readdir('./product', (err, data) => {
        if (err) {
            ...
            console.log(`Quantidade total de produtos: ${chalk.yellow.bold(allProductsObj.length)}`);
            ...
            allProductsObj.forEach((produto) => {
                ...
                fileSystem.readFile(`./product/${produto}`, (err, data) => {
                    ...
                    if (data) {
                        // Produtos representados em Objetos
                        ...
                        class ClassProduct {
                            ...
                            printProductName = () => { return this.name };
                            printProductPrice = () => { return this.price };
                        }
                        const myObjProduto = new ClassProduct(nome, price);

                        const namePrint = myObjProduto.printProductName();
                        const pricePrint = myObjProduto.printProductPrice();

                        let arrayProducts = [
                            { nome_produto: namePrint, valor_produto: pricePrint }
                        ];



                        console.log(` ${chalk.yellow.bold('->')} Nome do produto:  ${chalk.blue.bold(`${myObjProduto.printProductName()}`)}`);
                        console.log(` ${chalk.yellow.bold('->')} Valor do produto: ${chalk.green.bold(`R$ ${myObjProduto.printProductPrice()}`)}`);
                    };
...
// Function | calcular o pagamento:
function calculatePayment() {

    fileSystem.readFile(`./product/Moto.json`, (err, data) => {
        if (err) {
            console.log(err);
        };
        ...
        if (data) {
            const motoData = data;
            const motoObj = JSON.parse(motoData);
            const motoPrice = motoObj.price;

            return console.log(motoPrice);
        };
    });

    fileSystem.readFile(`./product/Carro.json`, (err, data) => {
        if (err) {
            console.log(err);
        };

        if (data) {
            const carroData = data;
            const carroObj = JSON.parse(carroData);
            const carroPrice = carroObj.price;

            console.log(carroPrice);
        };
    });
...
// Function | Questionar ao usuário se quer efetuar o pagamento:
function questionForUserPayment() {
    const questionForPayment = [
        {
            type: 'list',
    ...
    inquirer
        .prompt(questionForPayment)
        .then((answers) => {
            let responseQuestion = answers['questionPayment'];

            if (responseQuestion == 'Não') {
                return optionUser();
            } else {
                payment();
            };
